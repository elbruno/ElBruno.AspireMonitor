...
Archived: 2026-05-11T20:13:32.191-04:00
Entries older than 2026-05-04
...
## UI Layout & Presentation Decisions

### Working Folder Always Visible (2026-04-26)
- **Decision:** Working folder display in MainWindow is configuration state and MUST be visible regardless of Aspire connection status
- **Rationale:** 
  - Configuration values should always be visible to provide user confirmation
  - Enables troubleshooting when Aspire fails to connect
  - Aligns with industry standards (Docker Desktop, Git clients show config when offline)
  - Users expect persistent config display independent of runtime state
- **Status:** ✅ IMPLEMENTED (Han, 2026-04-26)
- **Implementation:**
  - Moved working folder TextBlock from IsConnected-bound Border to dedicated Grid.Row="1"
  - Added consistent working folder display to MiniMonitorWindow
  - Exposed WorkingFolder property in MiniMonitorViewModel
  - Layout pattern: Always-visible rows (title, working folder) → Conditional rows (connection/error state) → Always-visible rows (content, controls, footer)
- **Files:** 
  - src/ElBruno.AspireMonitor/Views/MainWindow.xaml (layout restructure)
  - src/ElBruno.AspireMonitor/Views/MiniMonitorWindow.xaml (added working folder section)
  - src/ElBruno.AspireMonitor/ViewModels/MiniMonitorViewModel.cs (property binding)
- **Alternatives Rejected:**
  - Show in error banner: Would clutter error messages with unrelated info
  - Show only when empty: Doesn't solve core problem (still hidden when set and disconnected)
  - Duplicate in both places: Code duplication and maintenance burden
- **Lessons Learned:** WPF Grid row management best practice — always update ALL downstream Grid.Row indices when inserting new rows; persistent state and runtime state should be in separate containers

---

**Merge Date:** 2026-04-26 18:45:02 UTC  
**Merged By:** Scribe (GitHub Copilot)  
**Status:** Ready for Visual Verification & Commit

---

## Recent Decisions (2026-04-27)

### System Tray Icon States — Reality Alignment (Chewie)

**Date:** 2026-04-27  
**Agent:** Chewie (DevRel/Docs)  
**Status:** ✅ IMPLEMENTED  
**Scope:** Documentation accuracy, README update

**Problem:** The README documented system tray status as a 4-state emoji model with "Orange/Partial" state, but the actual app implements 4 different states with "Not Running" (gray) instead.

**Decision:** Update all user-facing documentation to reflect the 4 real icon states:
1. ✅ Remove `🟠 Orange (Partial)` from all docs
2. ✅ Add `aspire_trayicon_norunning.png` (gray) as the 4th state
3. ✅ Replace emoji representations with actual embedded PNG icons in README
4. ✅ Use `<img src="./images/aspire_trayicon_*.png" width="24">` for size-controlled icons

**Rationale:**
- Documentation must match code. The actual StatusCalculator/PollingServiceState produces 4 outputs; documentation should reflect these 4 states exactly.
- "Not Running" (no instance found) is UX-distinctly different from "Error" (connection lost). Documenting them separately improves user comprehension.
- Embedded PNG icons are more trustworthy than emoji (emoji rendering varies by OS/platform).

**Implementation:**
- **Line 32 (Features table):** Changed emoji from 🟢🟡🟠🔴 to 🖼️; description: "Green/Yellow/Red/Gray"
- **Lines 57-66 (System Tray Status table):** Replaced emoji with HTML `<img>` tags with width="24" for all 4 states
- **Line 91 (System Tray usage):** Updated to "green=running, yellow=warning, red=error, gray=not running"
- **Commit:** f73b7be — `docs(readme): refresh System Tray Status with real app icons`

**Verification:**
- ✅ All 4 PNG icons exist in images/
- ✅ Icon descriptions match actual app behavior (no fictional states)
- ✅ "Not Running" properly distinguished from "Error"
- ✅ HTML sizing applied consistently
- ✅ README renders correctly on GitHub

**Pattern Established:**
When embedding status/system tray icons in documentation:
1. Always use actual PNG files (not emoji)
2. Embed as HTML `<img>` tags with sizing
3. Verify icon states match code before documenting
4. Use relative GitHub-friendly paths
5. Distinguish error vs. not-running vs. warning in descriptions

---

### README Framing Around Job-to-be-Done Workflow (Chewie)

**Date:** 2026-04-27  
**Owner:** Chewie (DevRel/Docs)  
**Status:** ✅ ACCEPTED & IMPLEMENTED  
**Related Issue:** README refinement per Bruno's clarification of actual product mental model

**Problem:** The original README positioned AspireMonitor as "a tool to monitor Aspire resources via endpoint URL entry," which misrepresented the actual mental model: User sets a working folder → App auto-discovers if Aspire instance is running there → If running, displays all deployed resources with clickable URLs. The README was endpoint-centric, not directory-centric.

**Solution:** Reframed README around the **job-to-be-done**: discovering and monitoring Aspire instances in a development directory.

**Key Changes:**
1. **Tagline:** "A Windows system tray monitor that discovers and displays Aspire running instances and their deployed resources"
2. **Workflow:** Explicit two-step workflow → Working Folder → Running Instance Detection → Live Resource Display
3. **What It Does:** Concrete workflow (Set folder → Auto-discover → Monitor)
4. **Features Table:** Added "Automatic Discovery", "Working Folder Display", "Single Tray Icon", 4-state model
5. **System Tray Status:** 4-icon states (availability, not CPU %)
6. **Quick Start:** "Set working folder when prompted" (not "Enter endpoint")
7. **Configuration Example:** `"workingFolder": "C:\\Projects\\MyAspireApp"` (not endpoint URL)
8. **Troubleshooting:** "Aspire instance not found?" (not "Can't connect to endpoint?")

**Rationale:**
- Mental Model Alignment: Product discovers Aspire instances in a directory by watching for running processes
- OllamaMonitor Precedent: Sibling project uses similar framing successfully
- Developer Workflow: Developers have a project directory, run `aspire run` there, want to see results
- Feature Emphasis: Developer-centric ("See all deployed services instantly") vs. technical ("Monitor CPU/memory with color thresholds")

**Link Verification:** All 7 internal documentation links verified as valid

**Completeness:**
- ✅ Job-to-be-done framing applied throughout
- ✅ Working folder as primary configuration concept
- ✅ Auto-discovery emphasized as differentiator
- ✅ All internal links verified
- ✅ Troubleshooting reframed around discovery

**Decision Impact:**
- **Marketing:** README now accurately represents product positioning (discovery-based, not endpoint-based)
- **Onboarding:** New users will expect "set folder" not "enter URL" (better UX match)
- **Documentation:** All guides consistently use working folder mental model
- **Tone:** Aligns with OllamaMonitor's honest, developer-centric approach

---

### Single-Instance Mutex Guard for WPF App (Han)

**Date:** 2026-04-27  
**Agent:** Han (Frontend Dev)  
**Status:** ✅ Implemented  
**Impact:** High (user experience, resource safety)

**Context:** ElBruno.AspireMonitor is a system tray application. Without single-instance enforcement, users could accidentally launch multiple instances, causing:
1. **Duplicate system tray icons** (confusing, cluttered)
2. **Redundant background polling** (wasted CPU/memory)
3. **Port/file conflicts**
4. **Ambiguous user interaction**

**Decision:** Implement **mutex-based single-instance enforcement** in `App.xaml.cs`:
1. Named mutex: `"ElBruno.AspireMonitor.SingleInstance"`
2. Check ownership **BEFORE** `base.OnStartup()` and **BEFORE** any UI/resource creation
3. If second instance detected: log message to file, then `Environment.Exit(0)` immediately
4. Hold mutex for entire app lifetime, release in `OnExit`

**Rationale:**

**Why Mutex (not other IPC)?**
- **Windows-native:** No external dependencies
- **Process-scoped:** Automatic cleanup on process termination
- **Named global resource:** Visible across all user sessions
- **Simple API:** Single boolean check for ownership

**Why Environment.Exit(0) (not Shutdown())?**
- `Shutdown()` is async and triggers OnExit lifecycle events
- Would allow resources to start initializing
- `Environment.Exit(0)` terminates immediately (synchronous) with clean exit code
- Prevents any UI thread work from being queued

**Why Check BEFORE base.OnStartup()?**
- `base.OnStartup()` triggers WPF framework initialization
- Checking first = zero footprint for second instance
- No resource leak

**Why Log to File?**
- Silent exit confuses users ("app won't start")
- Log file provides evidence of single-instance behavior
- Path: `%LocalAppData%\ElBruno.AspireMonitor\logs\app-{timestamp}.log`

**Alternatives Rejected:**
- ❌ Process name check: Race condition, doesn't work for different paths, no automatic cleanup
- ❌ TCP socket binding: Requires network permissions, port conflicts, overkill
- ❌ File lock: Lock persists on crash, permissions issues
- ❌ WPF Application.Current.MainWindow check: Too late, doesn't prevent second process

**Consequences:**

**Positive:**
- ✅ Zero duplicate tray icons (user clarity)
- ✅ Single polling service instance (resource efficiency)
- ✅ Consistent app state (no race conditions)
- ✅ Observable behavior (log file for debugging)
- ✅ Fast exit for second instance (~50ms, no UI thread startup)

**Negative:**
- ⚠️ Windows-specific (Mitigation: App is already WPF which is Windows-only)
- ⚠️ Silent exit may confuse users without log access

**Risks:**
- ⚠️ Mutex not released if app crashes hard (Mitigation: OS cleans up abandoned mutexes on process termination)
- ⚠️ Different user sessions could run separate instances (Mitigation: Expected behavior — each user should have their own instance)

**Testing:**
- Manual: Launch once → runs normally; launch again → exits immediately with log message; kill first → launch again → runs normally
- Automated: Code review + manual verification (would require mocking Mutex)
- Test suite: 260/260 unit tests passing ✅

**Success Criteria:**
- [x] Second instance exits within 100ms
- [x] Log file shows "Second instance detected" message
- [x] No duplicate tray icons observed
- [x] Test suite remains green (260/260)
- [x] Mutex released on normal app exit
- [x] Mutex released on crash (OS-level cleanup)

**Code Reference:** `src/ElBruno.AspireMonitor/App.xaml.cs`
...
**Merge Date:** 2026-04-27 19:47:23 UTC  
**Merged By:** Scribe (GitHub Copilot)  
...
---
## Archived on 2026-05-24T10:03:12.875-04:00
